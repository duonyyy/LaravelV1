                                    ...
                                    <tbody>
                                        @foreach( $listSlider as $key => $value)
                                            <tr>
                                                ...
                                                <td>{{ $value->title }}</td>
                                                <td>
                                                    <a href="{{route('admin.sliders.updateSlider', $value->id)}}" >
...
<script>
    ...
    modeDelete.addEventListener('show.bs.modal', function (event) {
        ...
        var dataIdSlider = button.getAttribute('data-id'); // lấy ID danh mục từ data-id

        var confirmDelete = document.querySelector('#deleteUserForm');
        // Cập nhật action của form với đường dẫn xóa danh mục đúng
        confirmDelete.setAttribute('action', '{{ route("admin.sliders.deleteSlider", "") }}/' + dataIdSlider);
